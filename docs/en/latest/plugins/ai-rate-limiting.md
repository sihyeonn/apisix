...
| enable_estimated_token_charging | boolean      | False    | false    |                                                         | When enabled, charge estimated prompt tokens in access phase and charge estimated completion tokens progressively (for streaming) to reduce bypass by closing streaming requests early. When disabled (default), charge exact token usage from upstream response usage in log phase (backward compatible). |
| prompt_tokens_estimator_divisor | integer      | False    | 4        | >0                                                      | Estimated prompt tokens = `ceil(utf8_bytes / divisor)`. Larger divisor results in fewer estimated tokens. Effective only when `enable_estimated_token_charging` is enabled. |
| completion_tokens_estimator_divisor | integer  | False    | 4        | >0                                                      | Estimated completion tokens = `ceil(utf8_bytes / divisor)`. Effective only when `enable_estimated_token_charging` is enabled. |
| stream_commit_tokens         | integer        | False    | 50       | >0                                                      | Commit completion token cost in batches to reduce limiter overhead during streaming. Effective only when `enable_estimated_token_charging` is enabled. |
